...
class SemanticKITTI_Nuscenes(PointCloudDataset):
    ...
    def loader(self, seq, frame):
        seq_path = osp.join(self.path,str(seq).zfill(2))
    ...
    def get_size_seq(self, seq_number):
        last = os.listdir(osp.join(self.path,'dataset/sequences',self.sequence[seq_number],'velodyne'))
        last.sort()
        file = last[-1]
        print("Last: ", file)
        file = file[:-4]
